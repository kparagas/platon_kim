...
from ._surface_solver import SurfaceSolver
...
class EclipseDepthCalculator:
    ...
    def compute_depths(self, t_p_profile, star_radius, planet_mass,
        ...
        plt.plot(lambda_grid*1e6, stellar_photon_fluxes)
        plt.show() 
        plt.close() 

        trap1 = np.trapz(y = stellar_photon_fluxes, x = lambda_grid)
        print(trap1)
        print(5.67e-8 * (3036**4))

        if not np.isinf(cloudtop_pressure):
            max_taus = np.max(taus, axis=1)
            if add_surface:
                emitted_f, refl_f, total_f = self.surf.compute_surface_flux(platon_wls = lambda_grid, flux_s = stellar_photon_fluxes)
            else:
