...
simulated_data = ascii.read('/Users/kimparagas/desktop/jwst_project/data/Pandexo/pandexo_output_csvs/LHS_3844b/granitoid_pandexo_5_transits_14_res_NEW.csv')
# simulated_data = ascii.read('/Users/kimparagas/desktop/jwst_project/retrieval_attempt/Metal-rich_test_data_NEW.csv')
eclipse_wavelengths_obs = np.array(simulated_data['Wavelength']) #m
ind = np.where(eclipse_wavelengths_obs < 12e-6)
# print(ind)
eclipse_wavelengths_obs = eclipse_wavelengths_obs[ind]
...
class CombinedRetriever:
    ...
    def _ln_like(self, params, transit_calc, eclipse_calc, fit_info, measured_transit_depths,
        ...
        surface_name = params_dict['surface_name']
        ...
        # CO2_abundance = params_dict['CO2_abund']
        if (params_dict['log_CO2_abund']) is None:
            CO2_abundance = None
        else: CO2_abundance = 10**params_dict['log_CO2_abund']
        # CO2_abundance = None
        # N2_abund = params_dict['N2_abund']
        ...
        try:
            ...
            if measured_eclipse_depths is not None:
                ...
                #     eclipse_wavelengths, calculated_eclipse_depths, _, _, _, eclipse_info_dict = eclipse_calc.compute_depths(
                #         t_p_profile, Rs, Mp, Rp, T_star, logZ, CO_ratio,
                #         scattering_factor=scatt_factor, scattering_slope=scatt_slope,
                #         cloudtop_pressure=cloudtop_P,
                #         T_spot=T_spot, spot_cov_frac=spot_cov_frac, custom_abundances = custom_abundances, 
                #         frac_scale_height=frac_scale_height, number_density=number_density,
                #         part_size = part_size, ri=ri, P_quench=P_quench, full_output=True, 
                #         surface_name = surface_name, surface = surface, stellar_blackbody = stellar_blackbody)
                # if surface_name is None:
                #     # print("used this but wasn't supposed to")
                #     # sys.exit()
                ...
                eclipse_wavelengths, calculated_eclipse_depths, eclipse_info_dict = eclipse_calc.compute_depths(
                    ...
                    frac_scale_height=frac_scale_height, number_density=number_density,
                    part_size = part_size, ri=ri, P_quench=P_quench, full_output=True, 
                    stellar_blackbody = stellar_blackbody, surface_name = surface_name, surface = surface, CO2_abund = CO2_abundance)
                # print(CO2_abund)
                calculated_eclipse_depths[np.logical_and(eclipse_wavelengths >= wfc3_start, eclipse_wavelengths <= wfc3_end)] += params_dict["wfc3_offset_eclipse"] 
                residuals = calculated_eclipse_depths - measured_eclipse_depths
                scaled_errors = error_multiple * measured_eclipse_errors

                # print(len(eclipse_wavelengths), len(calculated_eclipse_depths))
                # print(type(eclipse_wavelengths[0]))
                # print(type(calculated_eclipse_depths[0]))
                # print(np.array(eclipse_wavelengths_obs) - np.array(eclipse_wavelengths))
                # print(np.array(eclipse_wavelengths_obs))
                # print(np.array(eclipse_wavelengths))
                # sys.exit()

                # plt.scatter(np.array(eclipse_wavelengths), np.array(calculated_eclipse_depths), color = 'red')
                # plt.scatter(np.array(eclipse_wavelengths), np.array(measured_eclipse_depths), color = 'k')
                # plt.show()
                # plt.close()
                ln_likelihood += -0.5 * np.sum(residuals**2 / scaled_errors**2 + np.log(2 * np.pi * scaled_errors**2))                                
        ...
        if params_dict["profile_type"] == "parametric":
            ...
            # # Psurf = 10**params_dict["log_cloudtop_P"]
            # P3 = 10**params_dict["log_P3"]
            # T0 = 10**params_dict["T0"]
            # T3 = 10**params_dict["T3"]
            # ln_P2 = params_dict["alpha2"]**2*(params_dict["T0"]+np.log(P1/1e-10)**2/params_dict["alpha1"]**2 - params_dict["T3"]) - np.log(P1)**2 + np.log(P3)**2
            # ln_P2 /= 2 * np.log(P3/P1)
            # P2 = np.exp(ln_P2)
            # T2 = params_dict["T3"] - np.log(P3/P2)**2/params_dict["alpha2"]**2
            # if P3 != P:
            #     return -np.inf
            # if P1 > P3: 
            #     return -np.inf
            ...
            # if T0 >= T3: 
                # return -np.inf
        # if CO2_abund is not None and N2_abund is not None: 
        #     # print(CO2_abund, N2_abund)
        #     if CO2_abund + N2_abund > 1: 
        #         return -np.inf
    ...
    def get_default_fit_info(Rs, Mp, Rp, T=None, logZ=0, CO_ratio=0.53,
                             ...
                             frac_scale_height=1, custom_abundances = None,
                             log_number_density=-np.inf, log_part_size=-6,
                             n=None, log_k=-np.inf,
                             log_P_quench=-99,
                             wfc3_offset_transit=0, wfc3_offset_eclipse=0,
                             surface_name = None, surface = None, CO2_abund = None, N2_abund = None,
