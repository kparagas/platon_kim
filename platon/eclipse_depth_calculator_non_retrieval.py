...
temps ={'Metal-rich': 1024.7, 'Ultramafic': 991.48, 'Feldspathic': 887.36, 'Basaltic': 1015.6, \
    'Granitoid': 884.7, 'Clay': 818.34, 'Ice-rich silicate': 973.8, 'Fe-oxidized': 1018.6}
...
class EclipseDepthCalculator:
    ...
    def _get_binned_depths(self, depths, atmosphere_depths, surface_depths, stellar_spectrum, n_gauss=10):
    ...
    def compute_depths(self, t_p_profile, star_radius, planet_mass,
        ...
        intermediate_T = temps[surface_name]
        dr = atm_info["dr"]
        ...
        if not np.isinf(cloudtop_pressure):
            ...
            fluxes_from_cloud = -model_surface_spectra_table[surface_name] * (max_taus**2 * scipy.special.expi(-max_taus) + max_taus * np.exp(-max_taus) - np.exp(-max_taus)) 
        ...
        if full_output:
            ...
            return binned_wavelengths, binned_depths, binned_atm_depths, binned_surface_depths, atm_info
